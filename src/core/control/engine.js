...
(function (global) {
  ...
  class Engine {
    ...
    _evaluateWakeUp() {
      ...
      const lastModelIdx = historyTurns.findLastIndex(
        (t) =>
          t.role === Role.MODEL &&
          t.meta &&
          t.meta.type === TurnType.MODEL_THOUGHT,
      );
      ...
      const recentTurns =
        lastModelIdx === -1
          ? historyTurns
          : historyTurns.slice(lastModelIdx + 1);
    ...
    injectSystemEvent(actionType, message, meta = {}) {
      ...
      const turn = this.state.history.append(
        Role.SYSTEM,
        turnContent,
        turnMeta,
      );
    ...
    async _ping() {
      ...
      try {
        ...
        if (actions.hasLeak) {
          ...
          const warningTurn = this.state.history.append(
            Role.SYSTEM,
            warningMsg,
            {
    ...
    _dispatchActions(actions) {
      ...
      const sharedTurn = this.state.history.append(
        Role.SYSTEM,
        getSortedResults(),
        {
          type: TurnType.TOOL_EXECUTION,

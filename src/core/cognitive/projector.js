...
(function (global) {
  ...
  class BaseProjector {
    ...
    _buildSystemPrompt(state) {
      ...
      if (state.configManager) {
        ...
        try {
          if (state.vfs && state.vfs.exists("system/config/config.json")) {
            config = JSON.parse(
              state.vfs.readFile("system/config/config.json"),
            );
          }
        } catch (e) {
      ...
      // 2. プロンプト内のプレースホルダー置換
      let effectivePrompt = this.systemPrompt.replace(
        /{{language}}/g,
        language,
      );
      ...
      const sessionStart = firstTurn
        ? new Date(firstTurn.timestamp)
        : new Date();
  ...
  class GeminiProjector extends BaseProjector {
      ...
      if (Array.isArray(turn.content)) {
        ...
        if (turn.meta && turn.meta.type === "tool_execution") {
          ...
          for (const c of turn.content) {
            ...
            if (
              !c.output ||
              (!c.output.log && !c.output.media && !c.output.image)
            )
              continue;
    ...
    async _uploadToGemini(dataUrl, mimeType, apiKey, signal) {
      ...
      if (!uploadRes.ok) {
        ...
        throw new Error(
          `Binary upload failed (${uploadRes.status}): ${errText}`,
        );
      }
  ...
  // src/core/cognitive/projector.js 内のパース処理部分

  class GemmaProjector extends BaseProjector {
    constructor(adapter, options = {}) {
      this.adapter = adapter;
      this.isThinking = false; // 現在思考中かどうかのフラグ
      this.buffer = ""; // タグの途切れを処理するためのバッファ
      this.onUpdate = options.onUpdate || (() => {}); // UI更新用コールバック
    }

    /**
     * ストリームを開始し、Gemmaの出力を正規化して投影する
     */
    async project(messages, signal) {
      ...
      try {
        ...
        console.error("[Projector] Projection failed:", error);
        throw error;
      }
    }
    ...
    _processChunk(chunk) {
      ...
      // 1. Gemma 4 特有の開始タグを Itera 標準に変換
      ...
      // 3. (高度な処理) タグがチャンクを跨いで分割された場合への配慮
      // 例: チャック1 "...<|" / チャンク2 "think|>..."
      this.buffer += text;

      // Itera UIへ正規化されたテキストを流す
      // UI側はこの <thinking> タグを見て、サイドバーに表示するかメインに表示するかを判断する
      this.onUpdate(text, {
  ...
  global.Itera.Cognitive.GemmaProjector = GemmaProjector;
})(window);

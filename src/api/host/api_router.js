...
(function(global) {
	...
	class ApiRouter {
		...
		_registerHandlers() {
			...
			t.registerHandler('fs:append', async ({
				path,
				content,
				opts
			}) => {
			...
			t.registerHandler('fs:delete', async ({
				path,
				opts
			}) => {
			...
			t.registerHandler('fs:rename', async ({
				oldPath,
				newPath,
				opts
			}) => {
			...
			t.registerHandler('fs:copy', async ({
				srcPath,
				destPath,
				opts
			}) => {
			...
			t.registerHandler('fs:mkdir', async ({
				path,
				opts
			}) => {
			...
			t.registerHandler('fs:stat', async ({
				path
			}) => d.vfs.stat(path));
			t.registerHandler('fs:list', async ({
				path,
				opts
			}) => d.vfs.listFiles({
				path,
			...
			// 2. AI & History (ai)
			// ==========================================
			t.registerHandler('ai:ask', async ({
				text,
				opts
			}) => {
			...
			t.registerHandler('ai:task', async ({
				instruction,
				context,
				opts
			}) => {
			...
			// ==========================================
			t.registerHandler('sys:spawn', async ({
				path,
				opts
			}) => {
			...
			t.registerHandler('sys:kill', async ({
				pid
			}) => {
			...
			t.registerHandler('sys:broadcast', async ({
				eventName,
				payload
			}) => {
			...
			t.registerHandler('sys:capture', async ({
				pid
			}) => {
			...
			// ==========================================
			t.registerHandler('host:open_editor', async ({
				path
			}) => {
			...
			t.registerHandler('host:notify', async ({
				message,
				title
			}) => {
			...
			t.registerHandler('host:copy', async ({
				text
			}) => {
				try {
			...
			t.registerHandler('host:open_url', async ({
				url
			}) => {
			...
			const prepareFetchOptions = (url, options) => {
				...
				if (options?.credentialId) {
					...
					if (options.useProxy && !config.network?.allowCredentialsWithProxy) {
						throw new Error("[Net API] Security Error: Cannot use public proxy with credentials. Enable 'allowCredentialsWithProxy' in settings if using a trusted local proxy.");
					}
					if (d.configManager) {
						const config = d.configManager.get();
				...
				if (options?.useProxy) {
					const config = d.configManager.get();
					const proxyPrefix = config.network?.proxyUrl || 'https://corsproxy.io/?';
			...
			t.registerHandler('net:fetch', async ({
				url,
				options
			}) => {
				const {
			...
			t.registerHandler('net:download', async ({
				url,
				destPath,
				options
			}) => {
			...
			t.registerHandler('net:oauth', async ({
				providerId,
				authUrl,
				instructions
			}) => {
			...
			t.registerHandler('dev:location', async ({
				options
			}) => {
			...
			t.registerHandler('dev:vibrate', async ({
				pattern
			}) => {
			...
			t.registerHandler('tools:unregister', async ({
				name
			}, sourcePid) => {

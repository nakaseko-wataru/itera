...
(function (global) {
  ...
  const { State, Control, Cognitive, Bridge, Shell } = global.Itera;
  const { Core, Windowing, Panels, Modals, Services } = Shell;
  ...
  class ShellController {
    ...
    async init() {
      ...
      let initialFiles = savedSystem
        ? savedSystem.files
        : this.config.DEFAULT_FILES || {};
      ...
      if (savedSystem && initialFiles["system/config/config.json"]) {
        try {
          const cfg = JSON.parse(
            initialFiles["system/config/config.json"].content,
          );
      ...
      if (autoUpdate && this.config.DEFAULT_FILES) {
        Object.keys(this.config.DEFAULT_FILES).forEach((path) => {
          if (
            ...
            path.startsWith("system/lib/") ||
            path.startsWith("system/kernel/")
          ) {
            initialFiles[path] = {
      ...
      const renderer =
        Services && Services.LPMLRenderer ? new Services.LPMLRenderer() : null;

      if (!renderer)
        console.warn(
          "[Itera] LPMLRenderer not found. Chat formatting will be disabled.",
        );
      ...
      this._createLLM = () => {
        ...
        const model = "gemma4-32k:latest";
        this._updateModelStatus(model);
        const adapter = new Cognitive.GemmaAdapter(
          apiKey,
          model,
          {},
          this.state.logger,
        ...
        // Wrap the generateStream method to update MODEL status
        const originalGenerateStream = adapter.generateStream;
        adapter.generateStream = async (...args) => {
          this._updateModelStatus(`${model} (Active)`);
          try {
            const result = await originalGenerateStream.apply(adapter, args);
            return result;
          } finally {
            this._updateModelStatus(model);
          }
      ...
      const projector = new Cognitive.GeminiProjector(
      ...
      try {
        if (vfs.exists("system/config/services.json")) {
          const services = JSON.parse(
            vfs.readFile("system/config/services.json"),
          );
          if (Array.isArray(services)) {
            for (const svc of services) {
              if (svc.pid && svc.path) {
                await this.windowing.processManager.spawn(
                  svc.pid,
                  svc.path,
                  "background",
                );
      ...
      const performDailyMaintenance = () => {
        ...
        if (purged > 0) {
          console.log(
            `[System Cron] Daily maintenance completed. Purged ${purged} old files.`,
          );
    ...
    _bindEvents() {
      ...
      chat.on("send", async (text, attachments) => {
        ...
        for (const file of attachments) {
          const isText =
            ...
            file.name.match(/\.(js|json|md|txt|html|css|xml|yml)$/);
      ...
      explorer.on("open_file", (path, content) => {
        const BINARY_EXTS =
          /\.(png|jpg|jpeg|gif|webp|svg|ico|pdf|zip|mp3|mp4|wav|ogg)$/i;
      ...
      settings.on("factory_reset", async () => {
        try {
          ...
          if (!confirm("Automatic backup failed. Continue reset anyway?"))
            return;
        }
      ...
      this.engine.on("loop_stop", (data) => {
        ...
        if (data && data.reason === "error") {
          console.error(
            "[MainController] Loop stopped due to error:",
            data.error,
          );
        }
    ...
    _updateStorageUI(usage) {
      ...
      this.els.STORAGE_BAR.className =
    ...
    _triggerAutoSave() {
      ...
      if (this.els.SAVE_STATUS) {
        ...
        this.els.SAVE_STATUS.className =
          "text-[9px] text-warning italic transition opacity-100 scale-90 origin-left";
      }
      this.saveTimer = setTimeout(async () => {
        ...
        if (this.els.SAVE_STATUS) {
          ...
          this.els.SAVE_STATUS.className =
            "text-[9px] text-success italic transition opacity-100 scale-90 origin-left";
          setTimeout(
            () => this.els.SAVE_STATUS.classList.add("opacity-0"),
            2000,
          );
        }
      }, 1000);
    ...
    async refreshPreview(path) {
      ...
      const currentPath =
        this.windowing.processManager.processes.get("main")?.path ||
        "index.html";
      await this.windowing.processManager.spawn(
        "main",
        path || currentPath,
    ...
    async clearSession(options = {}) {
      ...
      if (
        restoreTools &&
        this.registry &&
        this.registry.getActiveDynamicToolDefinitions
      ) {
